...
def process_cell_counts(df):
    ...
    cell_summary["umi_counts_per_cell"] = (
        cell_summary["count"]["count"].values * cell_summary["count"]["mean"].values
    )
...
def _filter_counts(
    df, cell_summary, gene_summary, min_genes=200, min_cells=3, save_path=None
):
    ...
    filtered_genes = gene_summary["count"].loc[
        gene_summary["count"]["count"] > min_cells
    ]
    filtered_cells = cell_summary["count"].loc[
        cell_summary["count"]["count"] > min_genes
    ]

    df_filtered = df.loc[df.cell.isin(filtered_cells.index.values)].loc[
        df.gene.isin(filtered_genes.index.values)
    ]
    ...
    print(
        (num_cells_unfiltered - num_cells),
        "cells filtered. Remaining:",
        num_cells,
        "(",
        np.round(cell_ratio, 2),
        "%)",
    )
    print(
        (num_genes_unfiltered - num_genes),
        "genes filtered. Remaining:",
        num_genes,
        "(",
        np.round(gene_ratio, 2),
        "%)",
    )
